...
public class ServiceItems {
    ...
    public ArrayList<Item> parseItems(String jsonText,int idh){
        try {
            Items=new ArrayList<>();
    ...
    public ArrayList<Item> getAllItems(int idh){
        String url = Statics.BASE_URL+"/housing/"+idh+"/items";
        req.setUrl(url);
        req.setPost(false);
        req.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
            public void actionPerformed(NetworkEvent evt) {
                Items = parseItems(new String(req.getResponseData()),idh);
